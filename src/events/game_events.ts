...
const get_games_event = (socket: Socket) => {
  socket.on(GAME_EVENTS.GET_GAMES, async () => {
    try {
      ...
      const error_message =
        err instanceof ResponseError ? err.message : 'Error al obtener los partidos de futbol'
  ...
  socket.on(
    ...
    async (data: { game_id: string; score: [number, number] }) => {
      try {
        ...
        const error_message =
          err instanceof ResponseError ? err.message : 'Error al actualizar el marcador del partido'
        socket.emit(GAME_EVENTS.ERROR, {
          success: false,
          error: error_message,
...
const update_game_status_event = (socket: Socket) => {
  socket.on(
    GAME_EVENTS.GAME_STATUS_UPDATED,
  ...
  socket.on(
    ...
    async (data: {
      ...
      try {
        ...
        const error_message =
          err instanceof ResponseError ? err.message : 'Error al crear el ticket'
        socket.emit(GAME_EVENTS.ERROR, {
          success: false,
          error: error_message,
...
const update_curva_event = (socket: Socket) => {
  socket.on(
    GAME_EVENTS.UPDATE_CURVA,
    async (data: { game_id: string; curva_id: string; curva_updated: CurvaEntity }) => {
      try {
        ...
        const error_message =
          err instanceof ResponseError ? err.message : 'Error al actualizar la curva'
        socket.emit(GAME_EVENTS.ERROR, {
          success: false,
          error: error_message,
...
const close_curva_event = (socket: Socket) => {
  socket.on(GAME_EVENTS.CLOSE_CURVA, async (data: { game_id: string; curva_id: string }) => {
    try {
...
const end_game_event = (socket: Socket) => {
  socket.on(GAME_EVENTS.END_GAME, async (data: { game_id: string }) => {
    try {
      const { game_id } = data
      const game_service = new SoccerGameService()
      ...
      const error_message =
        err instanceof ResponseError ? err.message : 'Error al finalizar el partido'
      socket.emit(GAME_EVENTS.ERROR, {
        success: false,
        error: error_message,
