...
export class UserService {
  ...
  public async request_recover_password({
    email,
    identity,
  }: {
    email?: string
    ...
    try {
      if (!email && !identity)
        throw new ResponseError(400, 'Se debe proporcionar un correo o una identificacion')
  ...
  public async verify_recover_code({
    email,
    identity,
    code,
  }: {
    ...
    try {
      if (!email && !identity && !code)
        throw new ResponseError(
          400,
          'Se debe proporcionar un correo o una identificacion y un codigo de recuperación'
        )
      ...
      if (user.recover_code !== code)
        throw new ResponseError(400, 'Codigo de recuperación incorrecto')
    } catch (err) {
      if (err instanceof ResponseError) throw err
  ...
  public async verify_successful_recover_password({
    email,
    identity,
    code,
    new_password,
    ...
    try {
      if (!email && !identity && !code && !new_password)
        throw new ResponseError(
          400,
          'Se debe proporcionar un correo o una identificacion, un codigo de recuperación y una nueva contraseña'
        )
      const user = await UserModel.findOne({ $or: [{ email }, { identity }] })
      if (!user) throw new ResponseError(404, 'Usuario no encontrado')
      if (user.recover_code !== code)
  ...
  public async login_user({ email, password }: { email: string; password: string }) {
    try {
      if (!email || !password)
        throw new ResponseError(400, 'Se debe proporcionar un correo y una contraseña')
  ...
  public async update_user_profile({
    user_id,
    payload,
    avatar,
  }: {
  ...
  public async upload_profile_image({
    user_id,
    image,
  }: {
    user_id: string
  ...
  private async verify_hashed_password({
    password,
    hashed_password,
  }: {
    password: string
