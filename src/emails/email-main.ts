...
const send_welcome_email = async ({
  name,
  email,
  temp_pssw,
}: SendEmailProps) => {
  try {
    ...
    if (!response)
      throw new ResponseError(400, "Error al enviar el correo de bienvenida");
...
const send_welcome_admin_email = async ({
  admin_name,
  admin_email,
  temp_admin_pssw,
}: SendAdminEmailProps) => {
  try {
    ...
    if (!response)
      throw new ResponseError(
        400,
        "Error al enviar el correo de bienvenida de administrador"
      );
...
const send_welcome_staff_email = async ({
  staff_name,
  staff_email,
  temp_staff_pssw,
}: SendStaffEmailProps) => {
  try {
    ...
    if (!response)
      throw new ResponseError(
        400,
        "Error al enviar el correo de bienvenida de staff"
      );
...
const send_recovery_email = async ({
  user_name,
  user_email,
  recover_code,
}: SendRecoveryEmailProps) => {
  try {
    ...
    if (!response)
      throw new ResponseError(400, "Error al enviar el correo de recuperación");
...
const send_event_confirmation_email = async ({
  ...
  try {
    ...
    if (!response)
      throw new ResponseError(
        400,
        "Error al enviar el correo de confirmación de evento"
      );
...
const send_result_notification_email = async ({
  ...
  try {
    ...
    const replaced_content = loaded_html
      ...
      .replace(
        /{{prize_amount}}/g,
        prize_amount ? prize_amount.toString() : "0"
      );
    ...
    if (!response)
      throw new ResponseError(
        400,
        "Error al enviar el correo de notificación de resultado"
      );
...
const send_ticket_status_update_email = async ({
  ...
  try {
    ...
    if (!response)
      throw new ResponseError(
        400,
        "Error al enviar el correo de actualización de estado"
      );
